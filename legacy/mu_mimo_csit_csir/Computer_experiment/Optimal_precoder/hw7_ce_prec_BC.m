...
P = Nr;           % total power constraint for BC
...
for i = 1:length(subsets)
    ...
    [~, Cov_UL] = iterative_waterfill(H_sub, P, 100);

    %compute downlink covariance matrices
    Cov_DL = MAC_to_BC(H_sub, Cov_UL);
    ...
    % === Compute log2 det RHS  ===
    Sig = eye(Nr);
    for k = 1:num_users
        Sig = Sig + (snr_lin(k)/Nr) * H_sub(:,:,k)' * Cov_DL(:,:,k)* H_sub(:,:,k);
    end
    d = det(Sig);
...
% === Display results ===
fprintf('--- BC Capacity Bound for Each Subset (Eq. 8.115 with iterative waterfilling) ---\n');
for i = 1:length(subsets)
