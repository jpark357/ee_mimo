...
H_all = cat(3, H1, H3);
U_set = [1, 3];   % user 0 and user 2

snr_db = [10, 5, 2];
snr_lin_full = 10.^(snr_db / 10);            
snr_lin = [snr_lin_full(1), snr_lin_full(3)];  % [snr0, snr2]
I = eye(Nr);
P = Nt;

% === Prepare channel input for iterative waterfilling ===
H_sub = H_all(:, :, 1:2);  % H1, H3
...
C2_opt = real(log2(det(I + (snr_lin(2)/Nt) * H3 * Cov(:,:,2) * H3')));
Csum_opt = real(log2(det(I + ...
    (snr_lin(1)/Nt) * H1 * Cov(:,:,1) * H1' + ...
    (snr_lin(2)/Nt) * H3 * Cov(:,:,2) * H3')));

% === Corner points ===
corner0 = [C0_opt, Csum_opt - C0_opt];  % decode user 0 first
corner1 = [Csum_opt - C2_opt, C2_opt];  % decode user 2 first

x_poly = [0, 0, corner1(1), corner0(1), C0_opt];
y_poly = [0, C2_opt, corner1(2), corner0(2), 0];
